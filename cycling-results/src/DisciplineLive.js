...
const DisciplineLive = () => {
  ...
  useEffect(() => {
    fetch(`https://cycling-results-backend.onrender.com/api/events/${eventName}`)
      .then((response) => response.json())
      .then((jsonData) => {
        setEventData(jsonData);
        setEventDisplayName(jsonData.name);
        setGoogleSheetsId(jsonData.googleSheetsId);
                ...
                <React.Fragment key={i}>
                  <tr className={`${isBold ? "bold-row" : ""}${isPairEnd ? "madison-pair-border" : ""}`}>
                    {row.map((cell, j) => {
                      ...
                      const isWinner = typeof cell === "string" && cell.toLowerCase().includes("winner");
                      return isCountryCol ? ( <td key={j}><img src={getFlagUrl(cell)} alt={cell} className="flag-icon" /></td> ) : ( <td key={j} className={isWinner ? "winner-cell" : ""}>{cell}</td>
                      );
                    })}
                  </tr>
                </React.Fragment>
